...
/*Вспомогательные функции*/
  ...
  function chordsFunction(messageChatId) {
    var url = "http://bot.prort.ru/";
    request(url, function (error, response, body) {
      if (!error) {
        var $ = cheerio.load(body),
            getText = $('#chords').text();
         if (chordsStack.length > 4) {
           chordsStack = []; //Очищаем, если размер стэка больше 4
         }
   			chordsStack.push(getText);
        if (messageChatId) {
          bot.sendMessage(messageChatId, getText);
        }
      }
  ...
  function lyricsFunction(messageChatId) {
    var url = "http://bot.prort.ru/";
    request(url, function (error, response, body) {
      if (!error) {
        var $ = cheerio.load(body),
            getText = $('#lyrics').text();
          if (chordsStack.length > 4) {
            lyricsStack = []; //Очищаем, если размер стэка больше 4
          }
         lyricsStack.push(getText);
         if (messageChatId) {
           bot.sendMessage(messageChatId, getText);
         }
      }
...
/*Список часто-используемых переменных*/
  ...
  //отправляем аккорды в чат
  if (messageText.indexOf('/chords') === 0) {
    if (chordsStack.length > 0) {
			bot.sendMessage(messageChatId, chordsStack[chordsStack.length - 1]);
		} else {
      chordsFunction(messageChatId);
		}
  }

  //отправляем текст песен в чат
  if (messageText.indexOf('/lyrics') === 0) {
    if (lyricsStack.length > 0) {
			bot.sendMessage(messageChatId, lyricsStack[lyricsStack.length - 1]);
		} else {
      lyricsFunction(messageChatId);
		}
  }
